...
# Function to retrieve highfive animation URL from the API
def get_highfive_animation(api_token):
...
# Command handler for /highfive command
@app.on_message(filters.command("highfive") & ~filters.forwarded & ~filters.via_bot)
def highfive_command(client, message):
    try:
        ...
        gif_url = get_highfive_animation(api_token)

        if gif_url:
            msg = f"{sender} highfived {target}! 🙌"
