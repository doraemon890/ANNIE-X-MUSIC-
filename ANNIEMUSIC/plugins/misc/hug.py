...
# Function to retrieve hug animation URL from the API
def get_hug_animation(api_token):
...
# Command handler for /hug command
@app.on_message(filters.command("hug") & ~filters.forwarded & ~filters.via_bot)
def hug_command(client, message):
    try:
        ...
        gif_url = get_hug_animation(api_token)

        if gif_url:
            msg = f"{sender} hugged {target}! 🤗"
