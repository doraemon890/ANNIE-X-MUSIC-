...
# Function to retrieve kill animation URL from the API
def get_kill_animation(api_token):
...
# Command handler for /kill command
@app.on_message(filters.command("kill") & ~filters.forwarded & ~filters.via_bot)
def kill_command(client, message):
    try:
        ...
        gif_url = get_kill_animation(api_token)

        if gif_url:
            msg = f"{sender} killed {target}! 😵"
