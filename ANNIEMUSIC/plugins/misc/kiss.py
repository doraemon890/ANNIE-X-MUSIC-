...
# Function to retrieve kiss animation URL from the API
def get_kiss_animation(api_token):
...
# Command handler for /kiss command
@app.on_message(filters.command("kiss") & ~filters.forwarded & ~filters.via_bot)
def kiss_command(client, message):
    try:
        ...
        gif_url = get_kiss_animation(api_token)

        if gif_url:
            msg = f"{sender} kissed {target}! 😘"
