...
# Function to retrieve lick animation URL from the API
def get_lick_animation(api_token):
...
# Command handler for /lick command
@app.on_message(filters.command("lick") & ~filters.forwarded & ~filters.via_bot)
def lick_command(client, message):
    try:
        ...
        gif_url = get_lick_animation(api_token)

        if gif_url:
            msg = f"{sender} licked {target}! 😛"
